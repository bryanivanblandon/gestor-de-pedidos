...
export function addQuoteItemRow(item = {}) {
  ...
  row.innerHTML = `
    ...
    <select class="item-line-discount-type" aria-label="Tipo de descuento por línea"><option value="monto" ${(item.descuentoLineaTipo || 'monto') !== 'porcentaje' ? 'selected' : ''}>Desc/u C$</option><option value="porcentaje" ${item.descuentoLineaTipo === 'porcentaje' ? 'selected' : ''}>Desc %</option></select>
    <input class="item-line-discount" type="number" min="0" step="0.01" value="${Number(item.descuentoLineaValor || 0)}" placeholder="Desc. unit." />
...
function getItems() { return Array.from(document.querySelectorAll('#quote-items-container .item-row')).map(row => ({ cantidad:Number(row.querySelector('.item-qty').value||0), productoId: row.dataset.productId || '', codigo: row.dataset.productCode || '', tipoVenta: row.dataset.tipoVenta || 'unidad', descripcion: row.querySelector('.item-desc').value.trim(), ancho:Number(row.querySelector('.item-width').value||0), alto:Number(row.querySelector('.item-height').value||0), precio:Number(row.querySelector('.item-price').value||0), descuentoLineaTipo: row.querySelector('.item-line-discount-type')?.value || 'monto', descuentoLineaValor: Number(row.querySelector('.item-line-discount')?.value || 0), manual: row.dataset.manual === 'true' })).filter(i => i.cantidad > 0 && i.descripcion && (i.productoId || i.manual)); }
function totals() { const items=getItems(); const subtotal=items.reduce((s,i)=>s+lineItemBaseTotal(i),0); const descuentoLineas=items.reduce((s,i)=>s+lineItemDiscountAmount(i),0); const subtotalNetoLineas=Math.max(0,subtotal-descuentoLineas); const tipo=qs('#quote-discount-type').value; const valor=Math.max(0,Number(qs('#quote-discount').value||0)); const descuentoGeneral=tipo==='porcentaje'?Math.min(subtotalNetoLineas, subtotalNetoLineas*Math.min(valor,100)/100):Math.min(subtotalNetoLineas, valor); const descuento=descuentoLineas+descuentoGeneral; return {items, subtotal, descuentoLineas, descuentoGeneral, descuentoTipo:tipo, descuentoValor:valor, descuento, total:Math.max(0,subtotal-descuento)}; }
export function recalcQuoteForm() { const t=totals(); qs('#quote-subtotal').textContent=t.subtotal.toFixed(2); qs('#quote-total').textContent=t.total.toFixed(2); document.querySelectorAll('#quote-items-container .item-row').forEach(row=>{ const item={cantidad:Number(row.querySelector('.item-qty').value||0), tipoVenta:row.dataset.tipoVenta||'unidad', ancho:Number(row.querySelector('.item-width').value||0), alto:Number(row.querySelector('.item-height').value||0), precio:Number(row.querySelector('.item-price').value||0), descuentoLineaTipo:row.querySelector('.item-line-discount-type')?.value||'monto', descuentoLineaValor:Number(row.querySelector('.item-line-discount')?.value||0)}; row.querySelector('.line-total').textContent=lineItemTotal(item).toFixed(2); }); }
...
function quoteDiscountLabel(type = 'monto', value = 0, currency = 'C$') { const amount = Number(value || 0); if (!amount) return ''; return type === 'porcentaje' ? `${amount}%` : money(amount, currency); }

export function printQuoteTicket(id) {
  const q=state.cotizaciones.find(x=>x.id===id); if(!q) return;
  const c=calcPedido(q); const currency=q.moneda||'C$';
  const rows=(q.items||[]).map(i=>{ const base=lineItemBaseTotal(i); const disc=lineItemDiscountAmount(i); return `<tr><td>${Number(i.cantidad||0)}</td><td>${escapeHtml(i.descripcion||'')}${i.tipoVenta==='area_cm2'?`<br><small>${i.ancho}×${i.alto} cm</small>`:''}${disc?`<br><small>Desc. unitario: ${escapeHtml(quoteDiscountLabel(i.descuentoLineaTipo||'monto', i.descuentoLineaValor||0, currency))} (-${money(disc,currency)})</small>`:''}</td><td class="right"><small>Bruto ${money(base,currency)}</small><br>${money(lineItemTotal(i),currency)}</td></tr>`; }).join('');
