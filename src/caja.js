...
export function listenCaja() {
  const unsubShifts = onSnapshot(cajaTurnosRef, snap => {
    ...
    state.cajaTurnos = mergeCajaTurnos(state.cajaTurnos, remote);
    renderCaja();
  }, error => {
    console.error('Error escuchando cajaTurnos:', error);
    toast('No pude sincronizar caja con Firebase. Revisa reglas y conexión.');
    renderCaja();
  });
  const unsubConfig = onSnapshot(configRef, snap => {
    if (snap.exists()) state.config = { ...state.config, ...snap.data() };
    if (state.currentCashShiftId && Array.isArray(state.config.ignoredShiftIds)) {
      state.config.ignoredShiftIds = state.config.ignoredShiftIds.filter(id => id !== state.currentCashShiftId);
    }
    renderCaja();
  }, error => {
...
function isIgnoredShift(t = {}) {
  if (state.currentCashShiftId && t.id === state.currentCashShiftId) return false;
...
export function getActiveShift() {
  if (state.currentCashShiftId) {
    const current = state.cajaTurnos.find(t => t.id === state.currentCashShiftId);
    if (current && normalizeShiftStatus(current.estado) !== 'cerrado' && normalizeShiftStatus(current.estado) !== 'cerrada' && normalizeShiftStatus(current.estado) !== 'closed') {
      return current;
    }
  }
...
export async function openCashShift(event) {
  ...
  let remoteOpen = [];
  try {
    remoteOpen = await fetchOpenShiftsFromFirestore();
  } catch (error) {
    console.warn('No pude revisar turnos abiertos remotos, intentaré abrir caja:', error);
  }
  ...
  try {
    await setDoc(configRef, { tipoCambio, updatedAt: serverTimestamp() }, { merge: true });
    state.config = { ...state.config, tipoCambio };
  } catch (error) {
    console.warn('No pude guardar tipo de cambio, continúo con apertura local/remota:', error);
    state.config = { ...state.config, tipoCambio };
  }
  ...
  // Primero abre la pantalla localmente para que no se quede diciendo "Caja cerrada".
  state.currentCashShiftId = localId;
  try { localStorage.setItem('posCurrentCashShiftId', localId); } catch {}
  upsertLocalShift(localShift);

  try {
    const ref = await addDoc(cajaTurnosRef, { ...localShift, id: undefined, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    state.currentCashShiftId = ref.id;
    try { localStorage.setItem('posCurrentCashShiftId', ref.id); } catch {}
    state.cajaTurnos = state.cajaTurnos.filter(t => t.id !== localId);
    upsertLocalShift({ ...localShift, id: ref.id });
    qs('#cash-opening-amount').value = '0';
    toast('Caja abierta. Ya puedes registrar ventas, gastos y cerrar turno.');
  } catch (error) {
    console.error('No se pudo guardar la apertura en Firebase:', error);
    toast('Caja abierta solo en este navegador. Firebase no dejó guardar el turno; revisa firestore.rules y sesión.');
  }
}
...
export async function confirmCloseCashShift(event) {
  ...
  try {
    ...
    console.error('No pude cerrar en Firebase, cierro localmente:', error);
    toast('Caja cerrada localmente. Firebase no dejó guardar el cierre; revisa reglas/sesión.');
  }
  ...
  try { localStorage.removeItem('posCurrentCashShiftId'); } catch {}
