...
function snapshotShift() {
  ...
  const id = activeId();
  if (id) {
    const found = state.cajaTurnos.find(t => t.id === id && isOpenStatus(t.estado));
    if (found) {
      setStoredShift(found);
      return found;
    }
  }
  const remote = sortShifts(state.cajaTurnos.filter(t => isOpenStatus(t.estado)))[0] || null;
...
export async function confirmCloseCashShift(event) {
  ...
  const closedShift = { ...shift, estado: 'cerrado', cierre: counted.total, conteo: collectDenominations(), dolaresContados: counted.usd, tipoCambioCierre: counted.tc, diferencia, totales: totals, closedAtLocal: nowISO(), updatedAtLocal: nowISO() };
  upsertShift(closedShift);
  saveClosedLocal(closedShift);
  setStoredShift(null);
  closeDialog('#cash-close-dialog');
  renderCaja();
  await persistShift(closedShift, { silent: false });
...
export async function forceCloseActiveShift() {
  ...
  if (shift) {
    const totals = shiftTotals(shift);
    const closed = { ...shift, estado: 'cerrado', cierre: totals.efectivoEsperado, diferencia: 0, cierreAdministrativo: true, totales, closedAtLocal: nowISO(), updatedAtLocal: nowISO() };
    upsertShift(closed);
    saveClosedLocal(closed);
    setStoredShift(null);
    renderCaja();
    await persistShift(closed, { silent: true });
    toast('Caja cerrada administrativamente. Ya puedes abrir una nueva.');
    return;
  }
  try {
    const snap = await getDocs(cajaTurnosRef);
    const opened = snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(t => isOpenStatus(t.estado));
    for (const t of opened) await setDoc(doc(db, 'cajaTurnos', t.id), { estado: 'cerrado', cierreAdministrativo: true, updatedAt: serverTimestamp() }, { merge: true });
