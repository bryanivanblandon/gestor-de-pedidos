...
function attachCashHandlers() {
  ...
  if (closeBtn) closeBtn.onclick = event => { event.preventDefault(); closeCashShift(); };

  const forceBtn = qs('#cash-force-close');
  if (forceBtn) forceBtn.onclick = event => { event.preventDefault(); forceCloseActiveShift(); };

  const confirmBtn = qs('#cash-confirm-close');
  if (confirmBtn) confirmBtn.onclick = event => { event.preventDefault(); confirmCloseCashShift(event); };

  const closeForm = qs('#cash-close-form');
  if (closeForm) closeForm.onsubmit = event => { event.preventDefault(); confirmCloseCashShift(event); };
}
