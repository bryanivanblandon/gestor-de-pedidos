...
import { escapeHtml, money, todayISO, lineItemQtyForStock, BUSINESS_NAME } from './utils.js';
...
function buildKardexRows() {
  ...
  state.productos.forEach(p => {
    const movements = (p.kardex || []).map((k, index) => ({
      productoId: p.id,
      codigo: p.codigo || '',
      producto: p.nombre || '',
      ...
      nota: k.nota || '',
      index,
    })).sort((a, b) => String(a.fecha || '').localeCompare(String(b.fecha || '')) || a.index - b.index);

    let saldo = 0;
    movements.forEach(m => {
      const saldoAnterior = saldo;
      saldo += Number(m.cantidad || 0);
      rows.push({ ...m, saldoAnterior, saldoPosterior: saldo });
    });
  });
  return rows;
}

function getFilteredKardexRows() {
  ...
  return buildKardexRows()
    ...
    .sort((a, b) => String(b.fecha || '').localeCompare(String(a.fecha || '')) || String(a.producto).localeCompare(String(b.producto)) || b.index - a.index);
}
...
export function renderKardexGeneral() {
  ...
  const filtered = getFilteredKardexRows();
  ...
  const ajustes = filtered.filter(r => String(r.tipo || '').toLowerCase() === 'ajuste').length;
  ...
  container.innerHTML = `
    <div class="summary-grid compact-grid">
      ...
      <div class="summary-card"><span>Ajustes</span><strong>${ajustes}</strong></div>
    </div>
    ${filtered.length ? `<div class="report-table-wrap"><table class="table report-table"><thead><tr><th>Fecha</th><th>Código</th><th>Producto</th><th>Categoría</th><th>Tipo</th><th>Movimiento</th><th>Saldo anterior</th><th>Saldo después</th><th>Nota</th></tr></thead><tbody>${filtered.map(r => `<tr><td>${escapeHtml(r.fecha)}</td><td><strong>${escapeHtml(r.codigo)}</strong></td><td>${escapeHtml(r.producto)}</td><td>${escapeHtml(r.categoria || '-')}</td><td><span class="status ${r.cantidad < 0 ? 'pendiente' : 'entregado'}">${escapeHtml(r.tipo)}</span></td><td>${r.cantidad > 0 ? '+' : ''}${r.cantidad}</td><td>${r.saldoAnterior}</td><td><strong>${r.saldoPosterior}</strong></td><td>${escapeHtml(r.nota)}</td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">No hay movimientos que coincidan con los filtros.</div>'}
  `;
}

export function renderInventorySheet() {
  const container = qs('#inventory-sheet-table');
  if (!container) return;
  const rows = state.productos.slice().sort((a, b) => String(a.categoria || '').localeCompare(String(b.categoria || '')) || String(a.nombre || '').localeCompare(String(b.nombre || '')));
  const totalCosto = rows.reduce((s, p) => s + Number(p.costo || 0) * Number(p.stock || 0), 0);
  const totalVenta = rows.reduce((s, p) => s + Number(p.precio || 0) * Number(p.stock || 0), 0);
  container.innerHTML = rows.length ? `
    <div class="summary-grid compact-grid">
      <div class="summary-card"><span>Productos</span><strong>${rows.length}</strong></div>
      <div class="summary-card"><span>Costo total</span><strong>${money(totalCosto, 'C$')}</strong></div>
      <div class="summary-card"><span>Valor venta</span><strong>${money(totalVenta, 'C$')}</strong></div>
    </div>
    <div class="report-table-wrap"><table class="table report-table"><thead><tr><th>Código</th><th>Producto</th><th>Categoría</th><th>Tipo venta</th><th>Stock actual</th><th>Stock mín.</th><th>Costo</th><th>Precio</th><th>Total costo</th><th>Total venta</th></tr></thead><tbody>${rows.map(p => {
      const stock = Number(p.stock || 0);
      return `<tr><td><strong>${escapeHtml(p.codigo || '')}</strong></td><td>${escapeHtml(p.nombre || '')}</td><td>${escapeHtml(p.categoria || '-')}</td><td>${p.tipoVenta === 'area_cm2' ? 'cm²' : 'Unidad'}</td><td><strong>${stock}</strong></td><td>${Number(p.stockMin || 0)}</td><td>${money(p.costo || 0, 'C$')}</td><td>${money(p.precio || 0, 'C$')}</td><td>${money(Number(p.costo || 0) * stock, 'C$')}</td><td>${money(Number(p.precio || 0) * stock, 'C$')}</td></tr>`;
    }).join('')}</tbody></table></div>` : '<div class="empty">No hay productos para imprimir.</div>';
}
...
function openPrintDocument(title, bodyHtml) {
  ...
  win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>
    ...
    button { padding: 8px 12px; border: 0; border-radius: 8px; background: #111827; color: white; font-weight: 700; }
    @media print { .no-print { display: none; } }
  </style></head><body><div class="no-print"><button onclick="window.print()">Imprimir / guardar PDF</button><button onclick="window.close()">Cerrar</button></div>${bodyHtml}</body></html>`);
  win.document.close();
  win.focus();
}

export function printKardexGeneral() {
  const rows = getFilteredKardexRows();
  const title = 'Kardex filtrado';
  const body = `<h1>${BUSINESS_NAME} · ${title}</h1><p>Generado: ${new Date().toLocaleString()}</p>${rows.length ? `<table><thead><tr><th>Fecha</th><th>Código</th><th>Producto</th><th>Categoría</th><th>Tipo</th><th>Movimiento</th><th>Saldo anterior</th><th>Saldo después</th><th>Nota</th></tr></thead><tbody>${rows.map(r => `<tr><td>${escapeHtml(r.fecha)}</td><td>${escapeHtml(r.codigo)}</td><td>${escapeHtml(r.producto)}</td><td>${escapeHtml(r.categoria || '-')}</td><td>${escapeHtml(r.tipo)}</td><td>${r.cantidad > 0 ? '+' : ''}${r.cantidad}</td><td>${r.saldoAnterior}</td><td><strong>${r.saldoPosterior}</strong></td><td>${escapeHtml(r.nota)}</td></tr>`).join('')}</tbody></table>` : '<p>No hay movimientos para imprimir.</p>'}`;
  openPrintDocument(title, body);
}

export function printInventorySheet() {
  const rows = state.productos.slice().sort((a, b) => String(a.categoria || '').localeCompare(String(b.categoria || '')) || String(a.nombre || '').localeCompare(String(b.nombre || '')));
  const title = 'Hoja de inventario';
  const body = `<h1>${BUSINESS_NAME} · ${title}</h1><p>Generado: ${new Date().toLocaleString()}</p>${rows.length ? `<table><thead><tr><th>Código</th><th>Producto</th><th>Categoría</th><th>Tipo venta</th><th>Stock actual</th><th>Stock mín.</th><th>Costo</th><th>Precio</th><th>Total costo</th><th>Total venta</th></tr></thead><tbody>${rows.map(p => { const stock = Number(p.stock || 0); return `<tr><td>${escapeHtml(p.codigo || '')}</td><td>${escapeHtml(p.nombre || '')}</td><td>${escapeHtml(p.categoria || '-')}</td><td>${p.tipoVenta === 'area_cm2' ? 'cm²' : 'Unidad'}</td><td><strong>${stock}</strong></td><td>${Number(p.stockMin || 0)}</td><td>${money(p.costo || 0, 'C$')}</td><td>${money(p.precio || 0, 'C$')}</td><td>${money(Number(p.costo || 0) * stock, 'C$')}</td><td>${money(Number(p.precio || 0) * stock, 'C$')}</td></tr>`; }).join('')}</tbody></table>` : '<p>No hay productos para imprimir.</p>'}`;
  openPrintDocument(title, body);
}
