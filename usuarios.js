...
const ACTIVE_KEY = 'arca_pos_caja_activa_v1';
const CLOSED_KEY = 'arca_pos_cajas_cerradas_v1';
...
function isOpenStatus(status = '') { return ['abierto', 'abierta', 'activo', 'activa', 'open'].includes(normalizeStatus(status)); }
function readActiveShift() {
  const shift = safeParse(localStorage.getItem(ACTIVE_KEY), null);
  if (!shift || !shift.id || !isOpenStatus(shift.estado)) return null;
  return sanitizeShift(shift);
}
function writeActiveShift(shift) {
  if (!shift || !shift.id || !isOpenStatus(shift.estado)) {
...
function readClosedShifts() { return safeParse(localStorage.getItem(CLOSED_KEY), []); }
function saveClosedShift(shift) {
  const closed = [{ ...shift, estado: 'cerrado' }, ...readClosedShifts().filter(t => t.id !== shift.id)].slice(0, 100);
  localStorage.setItem(CLOSED_KEY, JSON.stringify(closed));
}
function sanitizeShift(shift = {}) {
  return {
    id: shift.id || `caja-${Date.now()}`,
...
function refreshCajaTurnosState() {
  const active = readActiveShift();
  const closed = readClosedShifts();
  const byId = new Map();
  remoteShifts.forEach(t => t?.id && byId.set(t.id, t));
  closed.forEach(t => t?.id && byId.set(t.id, t));
  if (active) byId.set(active.id, active);
  state.cajaTurnos = Array.from(byId.values());
...
async function persistShift(shift, { silent = true } = {}) {
  ...
  try {
    ...
    console.warn('Caja: guardado local, no sincronizado con Firebase:', error);
    if (!silent) toast('Caja guardada en este navegador. Firebase no permitió sincronizar ahora.');
    return false;
  }
}
function attachCashHandlers() {
  const map = [
    ['#cash-close', closeCashShift],
    ['#cash-force-close', forceCloseActiveShift],
    ['#cash-confirm-close', confirmCloseCashShift],
  ];
  map.forEach(([selector, handler]) => {
    const el = qs(selector);
    if (el) el.onclick = event => { event.preventDefault(); handler(event); };
  });
  const form = qs('#cash-close-form');
  if (form) form.onsubmit = event => { event.preventDefault(); confirmCloseCashShift(event); };
  document.querySelectorAll('.denom-input, #close-usd-amount').forEach(input => {
    input.oninput = () => renderCloseTotal();
  });
}
...
export function listenCaja() {
  ...
  const unsubShifts = onSnapshot(cajaTurnosRef, snap => {
    ...
    console.warn('Caja: no pude leer turnos remotos, sigo con caja local:', error);
    refreshCajaTurnosState();
    renderCaja();
  });
  ...
  }, error => console.warn('Caja: no pude leer configuración remota:', error));

  return () => { try { unsubShifts(); } catch {} try { unsubConfig(); } catch {} };
}

export function getActiveShift() { return readActiveShift(); }
...
export async function openCashShift(event) {
  ...
  refreshCajaTurnosState();
  renderCaja();
  await persistShift(shift, { silent: true });
...
async function finalizeClose({ administrativo = false } = {}) {
  ...
  try {
    ...
    const closedShift = sanitizeShift({
      ...shift,
      estado: 'cerrado',
    ...
    closedShift.estado = 'cerrado';
    closedShift.cierre = counted.total;
    closedShift.diferencia = diferencia;
    closedShift.totales = totals;
    saveClosedShift(closedShift);
    writeActiveShift(null);
    closeDialog('#cash-close-dialog');
    refreshCajaTurnosState();
    renderCaja();
    await persistShift({ ...closedShift, estado: 'cerrado' }, { silent: true });
...
export async function forceCloseActiveShift(event) {
  ...
  // Limpieza remota no bloqueante: intenta cerrar turnos abiertos antiguos, pero no afecta la pantalla.
  try {
    const snap = await getDocs(cajaTurnosRef);
    await Promise.all(snap.docs.map(d => {
      const data = d.data();
      if (!isOpenStatus(data.estado)) return Promise.resolve();
