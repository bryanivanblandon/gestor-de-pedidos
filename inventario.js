...
import { listenInventario, saveProduct, resetProductForm, editProduct, addInventoryMovement, showKardex, previewNextProductCode, renderKardexGeneral, printKardexGeneral, printInventorySheet } from './inventario.js';
...
function bindForms() {
  ...
  qs('#cash-confirm-close')?.addEventListener('click', confirmCloseCashShift);
  ...
  qs('#kardex-search')?.addEventListener('input', e => { state.kardexSearch = e.target.value; renderKardexGeneral(); });
  qs('#kardex-product-filter')?.addEventListener('change', e => { state.kardexProducto = e.target.value; renderKardexGeneral(); });
  qs('#kardex-type-filter')?.addEventListener('change', e => { state.kardexTipo = e.target.value; renderKardexGeneral(); });
...
function bindDynamicActions() {
  document.addEventListener('click', e => {
    ...
    if (target.id === 'cash-confirm-close') confirmCloseCashShift(e);
  });
